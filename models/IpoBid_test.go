...
func Test_CreateIpoBid(t *testing.T) {
	...
	ipoBidId2, err := CreateIpoBid(102, ipoStockId, 1, 10000)
	if err != nil {
		t.Fatalf("Errored in CreateIpoBid : %+v", err)
	}
	ipoBidId3, err := CreateIpoBid(103, ipoStockId, 1, 10000)
	if err != nil {
		t.Fatalf("Errored in CreateIpoBid : %+v", err)
	}
	ipoBidId4, err1 := CreateIpoBid(104, ipoStockId, 1, 1000000000) // fails because of not enough cash
	ipoBidId5, err2 := CreateIpoBid(101, ipoStockId, 1, 10000)      // fails because of >1 bid (and thus >1 slotquantity) for same user
	ipoBidId6, err3 := CreateIpoBid(102, ipoStockId, 2, 10000)      // fails because of >1 slotquantity
